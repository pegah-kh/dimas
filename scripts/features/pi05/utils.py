...
def get_feature_distribution(all_data, episode_list, success_only=True, each_n_steps=1, max_ep_per_task=None):
    all_features = []
    success = 0
    total_episodes = 0
    ...
    for episode in episode_list:
        ...
        for ep_idx in unique_episodes:
            ...
            ep_feature = np.array([ep_feature[i*each_n_steps:min((i+1)*each_n_steps, n)].mean() 
...
def get_success_labels(all_data, episode_list, success_only=True, each_n_steps=1, max_ep_per_task=None):
    all_success_labels = []
...
def get_hidden_repr(episode_data, layer_key=None, fm_time_step=None, max_ep_per_task=None):
    ...
    for ep_hook in hook_data:
        ...
        while robot_time_step < len(hidden_states):
            ...
            hs = hidden_states[robot_time_step][layer_key] # torch.Size([10 (flow matching steps), 10 (robot time steps), 1024]) for fm and torch.Size([1, 1, 2048]) for vlm
        ...
        all_repr = all_repr[:accumulated_len_reprs] # in case we overshoot the end due to the step of 10
    if len(all_repr) == 0:
        return None
...
def vlm_steering_generate_regression_with_classifier(layer_num, extraction_dir, episode_list,
    ...
    feature, _ = get_feature_distribution(all_data, episode_list, success_only=True, each_n_steps=10)
    ...
    success_labels = get_success_labels(all_data, episode_list, success_only=False, each_n_steps=10)
