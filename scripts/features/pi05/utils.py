...
def get_feature_distribution(
    all_data, episode_list, success_only=True, each_n_steps=1, max_ep_per_task=None
):
    all_features = []
    success = 0
    total_episodes = 0
    ...
    for episode in episode_list:
        ...
        for ep_idx in unique_episodes:
            ...
            ep_feature = feature_info[mask]
            ep_feature = ep_feature[:-1]

            n = len(ep_feature)
            n_chunks = math.ceil(n / each_n_steps)
            ep_feature = np.array(
                [
                    ep_feature[i * each_n_steps : min((i + 1) * each_n_steps, n)].mean()
...
def get_success_labels(
    ...
    for episode in episode_list:
        ...
        for ep_idx in unique_episodes:
            ...
            target_length = math.ceil(target_length / each_n_steps)
...
def get_hidden_repr(
    episode_data, layer_key=None, fm_time_step=None, max_ep_per_task=None
):
    ...
    for ep_hook in hook_data:
        ...
        accumulated_len_reprs += len(hidden_states)

        robot_time_step = 0
        while robot_time_step < len(hidden_states):
            hs = hidden_states[
                robot_time_step
            ][
                layer_key
            ]  # torch.Size([10 (flow matching steps), 10 (robot time steps), 1024]) for fm and torch.Size([1, 1, 2048]) for vlm
            ...
            if repr_i.shape[0] > 1:
                for i in range(repr_i.shape[0]):
                    all_repr.append(
                        repr_i[i].unsqueeze(0)
                    )  # this is to have separate representation for each of the tokens in flow matching of pi05
            else:
                all_repr.append(repr_i)
            robot_time_step += 10  # the states are each 10 steps, so we can skip ahead by 10 to get the next one without checking every single step

        all_repr = all_repr[
            :accumulated_len_reprs
        ]  # in case we overshoot the end due to the step of 10
    if len(all_repr) == 0:
        return None
...
def load_episode_data(extraction_dir, episode_list, suffix_episode="", flat=False):
    ...
    for episode in episode_list:
        if flat:
            path = os.path.join(
                extraction_dir, "videos", episode, "episode_output_data.pt"
            )
        else:
            path = os.path.join(
                extraction_dir,
                episode + suffix_episode,
                "videos",
                episode,
                "episode_output_data.pt",
            )
        all_data[episode] = torch.load(path)
    return all_data


def vlm_steering_generate_regression_with_classifier(
    layer_num,
    extraction_dir,
    episode_list,
    low_q=0.25,
    ...
    all_repr = []
    all_data = load_episode_data(
        extraction_dir, episode_list, suffix_episode="_mean_vlm"
    )
    ...
    feature, _ = get_feature_distribution(
        all_data, episode_list, success_only=True, each_n_steps=10
    )
    q_low_val = float(np.quantile(feature, low_q))
    q_high_val = float(np.quantile(feature, high_q))
    print(
        f"Feature quantiles — q_low ({low_q}): {q_low_val:.4f} | q_high ({high_q}): {q_high_val:.4f}"
    )

    success_labels = get_success_labels(
        all_data, episode_list, success_only=False, each_n_steps=10
    )
    ...
    train_idx = list(np.where(feature_train < q_low_val)[0]) + list(
        np.where(feature_train > q_high_val)[0]
    )

    labels_low = np.zeros(len(y_train))
    labels_low[np.where(feature_train > q_high_val)[0]] = 1
    labels_high = np.zeros(len(y_train))
    labels_high[np.where(feature_train < q_low_val)[0]] = 1
    ...
    for q_target_val, tag, clf in [
        (q_low_val, f"{FEATURE_TAG}_low", clf_low),
        (q_high_val, f"{FEATURE_TAG}_high", clf_high),
    ]:
...
def vlm_steering_generate_diff_means_with_classifier(
    layer_num,
    extraction_dir,
    episode_list,
    low_q=0.25,
    ...
    print(
        f"Layer {layer_num} | Slow: {len(low_feature_idx)} | Fast: {len(high_feature_idx)} | ‖diff‖: {np.linalg.norm(diff_to_low_feature):.4f}"
    )
    ...
    X_clf = np.concatenate(
        [repr_all[low_feature_idx], repr_all[high_feature_idx]], axis=0
    )
    is_fast = np.concatenate(
        [np.zeros(len(low_feature_idx)), np.ones(len(high_feature_idx))]
    )
    ...
    print(
        f"Layer {layer_num} | SVM  acc (low): {clf_low.score(X_clf, is_fast):.4f} | (high): {clf_high.score(X_clf, 1 - is_fast):.4f}"
    )
    ...
    for tag, clf, clf_vecs, steer_vecs, q_target in [
        (
            f"{FEATURE_TAG}_low",
            clf_low,
            classifier_vecs_to_low_feature,
            steering_vecs_to_low_feature,
            q_low_val,
        ),
        (
            f"{FEATURE_TAG}_high",
            clf_high,
            classifier_vecs_to_high_feature,
            steering_vecs_to_high_feature,
            q_high_val,
        ),
    ]:
...
def fm_steering_generate_regression_with_classifier(
    layer_num,
    extraction_dir,
    episode_list,
    steps=list(range(10)),
    ...
    classifier_vecs = torch.zeros([num_steps, 1, 1025])  # 1 for bias, 1024 for coef
    layer = f"model.paligemma_with_expert.gemma_expert.model.layers.{layer_num}.mlp_gated_residual"

    steering_vecs = torch.zeros([num_steps, 1, 1024])
    ...
    feature, _ = get_feature_distribution(
        all_data, train_episodes, success_only=True, each_n_steps=1
    )
    q_low_val = float(np.quantile(feature, low_q))
    q_high_val = float(np.quantile(feature, high_q))
    print(
    ...
    success_labels = get_success_labels(
        all_data, train_episodes, success_only=False, each_n_steps=1
    )
    ...
    for j, step in enumerate(steps):
        ...
        for episode in train_episodes:
            all_repr.append(
                get_hidden_repr(all_data[episode], fm_time_step=step, layer_key=layer)
            )
        ...
        labels_low_feature = np.zeros(len(y_train))
        labels_low_feature[high_feature_idx] = 1
        labels_high_feature = np.zeros(len(y_train))
        labels_high_feature[low_feature_idx] = 1
        ...
        print(
            f"Step {step} | SVM acc (low/fast=1): {clf_low.score(X_train[train_idx], labels_low_feature[train_idx]):.4f} "
            f"| (high/slow=1): {clf_high.score(X_train[train_idx], labels_high_feature[train_idx]):.4f}"
        )

    for q_target_val, tag, clfs in [
        (q_high_val, f"{FEATURE_TAG}_high", classifiers_high),
...
def get_OT_flow_matching(
    repr, feature, low_thresh, high_thresh, ot_type="lowrank_sinkhorn", plot=False
):
    ...
    if ot_type == "linear":
        ...
        val = ot.lowrank.lowrank_sinkhorn(
            X_s=Xs, X_t=Xt, reg=0.0001, numItermax=5000, log=True
        )
...
def fm_steering_generate_OT(
    ...
    # ── Speed distribution ────────────────────────────────────────────────
    feature_train, _ = get_feature_distribution(
        all_data,
        train_episodes,
        success_only=True,
        each_n_steps=1,
        max_ep_per_task=max_ep_per_task,
    )
    has_test = bool(test_episodes)
    if has_test:
        feature_test, _ = get_feature_distribution(
            all_data,
            test_episodes,
            success_only=True,
            each_n_steps=1,
    ...
    print(
        f"Thresholds -> low={low_thresh:.4f} (q{low_quantile}) | high={high_thresh:.4f} (q{high_quantile})"
    )

    # ── Success labels ────────────────────────────────────────────────────
    success_train = get_success_labels(
        all_data,
        train_episodes,
        success_only=False,
        each_n_steps=1,
    ...
    if has_test:
        success_test = get_success_labels(
            all_data,
            test_episodes,
            success_only=False,
            each_n_steps=1,
    ...
    for j, step in enumerate(steps):
        ...
        # ── Fit model ─────────────────────────────────────────────────────
        if mode == "classifier":
            train_idx = list(np.where(feature_train < low_thresh)[0]) + list(
                np.where(feature_train > high_thresh)[0]
            )
            ...
            if has_test:
                test_idx = list(np.where(feature_test < low_thresh)[0]) + list(
                    np.where(feature_test > high_thresh)[0]
                )
                ...
                print(
                    f"Step {step} | Classifier accuracy: {acc:.1f}% | mean prob_fast: {prob_fast_test.mean():.3f}"
                )
        ...
        ot_details = get_OT_flow_matching(
            repr_all, feature_all, low_thresh=low_thresh, high_thresh=high_thresh
        )
    ...
    torch.save(
        {
            ...
            "ot_couplings": [
                torch.tensor(c).float() if not isinstance(c, int) else c
                for c in couplings
            ],
            "ot_Xs": [
                torch.tensor(x).float() if not isinstance(x, int) else x
                for x in Xs_list
            ],
            "ot_Xt": [
                torch.tensor(t).float() if not isinstance(t, int) else t
                for t in Xt_list
            ],
        },
        save_path,
