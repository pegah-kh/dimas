...
from scripts.features.smolvla.utils import (vlm_steering_generate_regression_with_classifier,
...
def build_parser():
    p = argparse.ArgumentParser(description="EEF height displacement (|delta_z|) probe & steering analysis")
    ...
    s = sub.add_parser("train-regression-vlm-clf", help="Train Ridge regression steering vector + SVM gate (VLM layers)")
    ...
    s = sub.add_parser("train-diff-means-vlm-clf", help="Diff of means steering vector + SVM gate (VLM layers)")
    s.add_argument("--low-q",  type=float, default=0.25)
    s.add_argument("--high-q", type=float, default=0.75)
    s.add_argument("--svm-C",  type=float, default=0.1)

    s = sub.add_parser("train-regression-fm-clf", help="Train FM height regression steering vector with sklearn SVM gate")
    ...
    sub.add_parser("nothing", help="do nothing")
...
def main():
    ...
    layer_num      = layer_nums[0] if layer_nums else None
