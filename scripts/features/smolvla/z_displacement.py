...
def build_parser():
    p = argparse.ArgumentParser(
        description="EEF height displacement (|delta_z|) probe & steering analysis"
    )
    ...
    s = sub.add_parser(
        ...
        help="Train Ridge regression steering vector + SVM gate (VLM layers)",
    )
    ...
    s = sub.add_parser(
        ...
        help="Diff of means steering vector + SVM gate (VLM layers)",
    )
    s.add_argument("--low-q", type=float, default=0.25)
    s.add_argument("--high-q", type=float, default=0.75)
    s.add_argument("--svm-C", type=float, default=0.1)
    ...
    s = sub.add_parser(
        ...
        help="Train FM height regression steering vector with sklearn SVM gate",
    )
