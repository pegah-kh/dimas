...
def get_feature_distribution(
    all_data, episode_list, success_only=True, max_ep_per_task=None
):
    all_features = []
    success = 0
    total_episodes = 0
...
def fm_steering_generate_OT(
    ...
    feature_train, _ = get_feature_distribution(
        all_data, train_episodes, success_only=True, max_ep_per_task=max_ep_per_task
    )
    has_test = bool(test_episodes)
    if has_test:
        feature_test, _ = get_feature_distribution(
            all_data, test_episodes, success_only=True, max_ep_per_task=max_ep_per_task
        )
    ...
    success_train = get_success_labels(
        all_data, train_episodes, success_only=False, max_ep_per_task=max_ep_per_task
    )
    if has_test:
        success_test = get_success_labels(
            all_data, test_episodes, success_only=False, max_ep_per_task=max_ep_per_task
        )
