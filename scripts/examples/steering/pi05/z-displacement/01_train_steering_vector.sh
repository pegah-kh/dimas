...
# from the FM extraction produced by extraction/pi05/FM/extract_baseline.sh.
#
...
# — see fm_steering_generate_OT in scripts/features/pi05/z_displacement.py:
# as soon as any task ends up in its internal test_episodes split, it gets
...
# methods exist in scripts/features/pi05/z_displacement.py and are worth
# trying as alternatives once you're comfortable with this pipeline:
#   - train-regression-fm-clf   (Ridge regression + SVM gate, FM layers)
#   - train-regression-vlm-clf  (Ridge regression + SVM gate, VLM layers —
#                                 needs extraction/pi05/VLM data instead)
#   - train-diff-means-vlm-clf  (difference of means, VLM layers)
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
XL_VLAS_ROOT="${XL_VLAS_ROOT:-$SCRIPT_DIR/../../../../..}"
HEIGHT_SCRIPT="$XL_VLAS_ROOT/scripts/features/pi05/z_displacement.py"
