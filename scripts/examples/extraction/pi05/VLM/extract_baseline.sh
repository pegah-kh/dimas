...
# few un-steered PI-0.5 rollouts. This is the "layer group 0" data used by
# scripts/features/pi05/speed.py's/z_displacement.py's VLM-based training
...
LAYERS=(14 15 16 17)            # late layers — gemma_2b VLM backbone only has 18 layers (0-17)

EXTRACTION_DIR="${EXTRACTION_DIR:-./results/examples/pi05/${SUITE}}"

# Build the modules_to_hook JSON array, e.g. [["model.paligemma_with_expert.paligemma.model.language_model.layers.14.mlp_gated_residual", ...]]
MODULES="["
for L in "${LAYERS[@]}"; do
    MODULES="${MODULES}\"model.paligemma_with_expert.paligemma.model.language_model.layers.${L}.mlp_gated_residual\","
done
MODULES="${MODULES%,}]"
MODULES="[${MODULES}]"
